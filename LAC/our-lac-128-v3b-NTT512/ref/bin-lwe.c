...
int poly_mul(const uint8_t *a, const uint8_t *s, uint8_t *b, unsigned int vec_num)
{
	...
	for(i=0;i<loop;i++)
	{
		b[i*2  ]=((( sum1[i]     &0xffff)-( sum2[i]     &0xffff)));
		b[i*2+1]=((((sum1[i]>>16)&0xffff)-((sum2[i]>>16)&0xffff)));
	}
...
int poly_aff(const uint8_t *a, const uint8_t *s, uint8_t *e, uint8_t *b, unsigned int vec_num)
{
	...
	for(i=0;i<loop;i++)
	{
		b[i*2  ]=((( sum1[i]     &0xffff)-( sum2[i]     &0xffff)+e[2*i] ));
		b[i*2+1]=((((sum1[i]>>16)&0xffff)-((sum2[i]>>16)&0xffff)+e[2*i+1]));
	}
